...
**شرح سناریو:**
کاربر `alice` فرایند خرید (`purchase`) را آغاز می‌کند و آن را به‌صورت هم‌زمان به دو کاربر `mortenaho` و `cara` ارجاع می‌دهد. با تکمیل وظیفه توسط هر دو نفر، وضعیت `allCompleted` برابر `true` می‌شود. سپس یک ارجاع گروهی به تیم حقوقی (`legal`) ثبت شده و کاربر `mortenaho` وظیفه را به خود تخصیص داده (Claim) و تکمیل می‌کند.
