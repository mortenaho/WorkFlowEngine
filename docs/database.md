...
مرجع اصلی ساختار و اسکیمای پایگاه داده، کلاس `PostgresStore` در مسیر `src/TaskFlow.Infrastructure/Persistence/PostgresStore.cs` است. عملیات مهاجرت (Migration) به‌صورت خودکار در نخستین اتصال انجام می‌پذیرد و نیازی به ابزار مهاجرت خارجی یا فایل‌های مجزای SQL وجود ندارد.
...
اطلاعات هویتی کاربران و عضویت در گروه‌ها در دیتابیس ذخیره نمی‌شوند. اگر دایرکتوری پیکربندی نشود، شناسه‌ها مات هستند (`OpenDirectory`). در غیر این صورت از `IDirectory` (`StaticDirectory` یا LDAP/SSO) تأمین می‌گردند.
...
- **کلیدهای امنیتی API:** از متغیرهای محیطی (`WF_API_KEYS`) خوانده می‌شود و در دیتابیس ذخیره نمی‌گردد. خارج از محیط Development این متغیر اجباری است. این کلید باید فقط روی بک‌اند (BFF) بماند و از مرورگر / React ارسال نشود؛ شرح معماری: [architecture.md](architecture.md#api-key-architecture).
