...
echo "== start =="
START=$(json sara -X POST "$BASE/v1/processes/start" \
...
echo "== refer to mortenaho + tina =="
REF=$(json sara -X POST "$BASE/v1/assignments" -d "$(cat <<EOF
{"definitionKey":"$DEF","parentInstanceId":"$ROOT","title":"تأیید موازی","to":{"kind":"users","ids":["mortenaho","tina"]}}
EOF
)")
echo "$REF"
...
TINA_TASK=$(echo "$REF" | python3 -c 'import json,sys; d=json.load(sys.stdin);
print(next(t["id"] for t in d["tasks"] if t["assigneeId"]=="tina"))')

echo "== inbox mortenaho =="
json mortenaho "$BASE/v1/tasks?user=mortenaho"
echo
echo "== completion before =="
json sara "$BASE/v1/instances/$RID/completion"
echo
...
echo
echo "== tina complete =="
json tina -X POST "$BASE/v1/tasks/$TINA_TASK/complete" -d '{"note":"ok"}'
echo
...
json sara -X POST "$BASE/v1/assignments" -d "$(cat <<EOF
{"definitionKey":"$DEF","parentInstanceId":"$ROOT","title":"بررسی حقوقی","to":{"kind":"group","id":"legal"}}
EOF
)"
echo
echo "== inbox group legal =="
json sara "$BASE/v1/tasks?group=legal"
echo

echo "== sara processes (open) =="
json sara "$BASE/v1/users/sara/processes?state=open"
echo

echo "== new process: complete-and-end =="
START2=$(json sara -X POST "$BASE/v1/processes/start" \
  -d '{"processKey":"purchase","initiator":"sara"}')
ROOT2=$(echo "$START2" | python3 -c 'import json,sys; print(json.load(sys.stdin)["instanceId"])')
REF2=$(json sara -X POST "$BASE/v1/assignments" -d "$(cat <<EOF
{"definitionKey":"$DEF","parentInstanceId":"$ROOT2","title":"تأیید نهایی","to":{"kind":"users","ids":["mortenaho","tina"]}}
EOF
)")
...
echo
json sara "$BASE/v1/users/sara/processes?state=closed"
echo
