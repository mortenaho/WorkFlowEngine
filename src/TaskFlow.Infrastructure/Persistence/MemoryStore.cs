...
public sealed class MemoryStore : IStore
{
    ...
    public Task<Definition?> GetDefinitionByKey(string tenantId, string key, CancellationToken cancellationToken = default)
    {
        lock (_mu)
        {
            ...
            {
                if (Tenant.Normalize(d.TenantId) == tenantId && d.Key == key)
                {
    ...
    public Task<IReadOnlyList<ProcessInstance>> ListRootInstances(string tenantId, string processKey, CancellationToken cancellationToken = default)
    {
        lock (_mu)
        {
            ...
            var outList = _instances.Values
                .Where(inst => Tenant.Normalize(inst.TenantId) == tenantId
    ...
    public Task<IReadOnlyList<ProcessInstance>> ListRootInstancesByInitiator(string tenantId, string initiator, CancellationToken cancellationToken = default)
    {
        lock (_mu)
        {
            tenantId = Tenant.Normalize(tenantId);
    ...
    public Task<IReadOnlyList<ProcessInstance>> ListChildInstances(string parentId, CancellationToken cancellationToken = default)
    {
        lock (_mu)
        {
            var outList = _instances.Values
                .Where(inst => inst.ParentInstanceId == parentId)
                .Select(i => i.Clone())
    ...
    {
        if (!string.IsNullOrEmpty(f.TenantId) && Tenant.Normalize(t.TenantId) != Tenant.Normalize(f.TenantId))
            return false;
